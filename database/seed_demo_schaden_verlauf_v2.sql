...
begin
  ...
  -- "created"-Ereignis fuer die Demo-Meldungen
  ...
  where t.title in (
      'Wasserfleck an der Decke im Treppenhaus','Tropfender Wasserhahn in der Gemeinschaftskueche',
      'Kellerlicht im Gang defekt','Loses Rinnengitter vor WE23','Fahrradkeller-Tuer schliesst nicht')
    and not exists (select 1 from schaden_events e where e.ticket_id = t.id and e.kind = 'created');

  -- Prioritaet / Behebungsweg passend zum Status ergaenzen (nur wenn noch leer,
  -- damit eigene Test-Klicks nicht ueberschrieben werden)
  update schaden_tickets set priority = 'niedrig'
    where title = 'Wasserfleck an der Decke im Treppenhaus' and priority is null;
  update schaden_tickets set priority = 'mittel'
    where title = 'Tropfender Wasserhahn in der Gemeinschaftskueche' and priority is null;
  update schaden_tickets set priority = 'mittel', repair_mode = 'handwerker', handwerker_info = 'Elektro Schulz, Termin am Freitag'
    where title = 'Kellerlicht im Gang defekt' and repair_mode is null;
  update schaden_tickets set priority = 'niedrig', repair_mode = 'eigenleistung', resources_available = true
    where title = 'Loses Rinnengitter vor WE23' and repair_mode is null;
  update schaden_tickets set priority = 'hoch'
    where title = 'Fahrradkeller-Tuer schliesst nicht' and priority is null;

  -- Verlauf: Statuswechsel / Prioritaet / Behebungsweg als Zeitstrahl (durch das Team)
  insert into schaden_events (ticket_id, kind, detail, created_by, created_by_name, created_at)
  select t.id, v.kind, v.detail, creator, creator_nm, t.created_at + v.delay
  from schaden_tickets t
  join (values
    ('Wasserfleck an der Decke im Treppenhaus',          interval '2 hours',        'priority', 'Prioritaet: Niedrig'),
    ('Tropfender Wasserhahn in der Gemeinschaftskueche', interval '3 hours',        'priority', 'Prioritaet: Mittel'),
    ('Tropfender Wasserhahn in der Gemeinschaftskueche', interval '1 day',          'status',   'Status: In Begutachtung'),
    ('Kellerlicht im Gang defekt',                       interval '3 hours',        'status',   'Status: In Begutachtung'),
    ('Kellerlicht im Gang defekt',                       interval '1 day',          'status',   'Status: Zur Behebung freigegeben'),
    ('Kellerlicht im Gang defekt',                       interval '1 day 2 hours',  'field',    'Behebungsweg: Handwerker'),
    ('Kellerlicht im Gang defekt',                       interval '2 days',         'status',   'Status: In Behebung'),
    ('Loses Rinnengitter vor WE23',                      interval '3 hours',        'status',   'Status: In Begutachtung'),
    ('Loses Rinnengitter vor WE23',                      interval '1 day',          'status',   'Status: Zur Behebung freigegeben'),
    ('Loses Rinnengitter vor WE23',                      interval '1 day 2 hours',  'field',    'Behebungsweg: Eigenleistung'),
    ('Loses Rinnengitter vor WE23',                      interval '2 days',         'status',   'Status: In Behebung'),
    ('Loses Rinnengitter vor WE23',                      interval '3 days',         'status',   'Status: Erledigt'),
    ('Fahrradkeller-Tuer schliesst nicht',               interval '2 hours',        'priority', 'Prioritaet: Hoch')
  ) as v(title, delay, kind, detail) on v.title = t.title
  where not exists (select 1 from schaden_events e where e.ticket_id = t.id and e.kind = v.kind and e.detail = v.detail);

  -- Ein paar oeffentliche Kommentare fuer einen lebendigen Verlauf
  insert into schaden_comments (ticket_id, body, is_internal, created_by, created_by_name, created_at)
  select t.id, v.body, false, creator, creator_nm, t.created_at + v.delay
  from schaden_tickets t
  join (values
    ('Wasserfleck an der Decke im Treppenhaus',          interval '4 hours',       'Danke fuer die Meldung - wir behalten den Fleck im Auge und pruefen die Ursache.'),
    ('Tropfender Wasserhahn in der Gemeinschaftskueche', interval '1 day 3 hours', 'Begutachtung fuer naechste Woche eingeplant.'),
    ('Kellerlicht im Gang defekt',                       interval '2 days 2 hours','Elektriker kommt am Freitag vorbei.'),
    ('Loses Rinnengitter vor WE23',                      interval '3 days 1 hour', 'Neues Gitter eingebaut - erledigt. Danke fuers Melden!')
  ) as v(title, delay, body) on v.title = t.title
  where not exists (select 1 from schaden_comments c where c.ticket_id = t.id and c.body = v.body);
